...
def main():
    ...
    if os.path.isfile(input_path): # parse individual protein files
        ...
        if Path("book.txt").is_file(): # if we already have a book.txt
            ...
            response = input("book.txt found! Run protein files identified in book.txt? (yes/no): ").strip().lower()
            if response == "yes" or response == "y":
            ...
            response = input("You can remove protein files to be run by editing book.txt if desired, and resume by running bb again. Run all protein files immediately? (yes/no): ").strip().lower()
            if response == "yes" or response == "y":
                if existing_dirs:
                    if not prompt_and_cleanup(existing_dirs):
                        sys.exit(1)
