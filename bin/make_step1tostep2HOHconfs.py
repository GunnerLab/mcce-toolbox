...
# An mandatory argumment is a PDB file containing one water oxygen atom. Two hydrogen atoms are added this oxygen to create a water molecule
...
def add_Hs(O_coord, bond_length=0.96, bond_angle=104.5):
    ...
    O  = ['{:07.3f}'.format(O_coord[0]),  '{:07.3f}'.format(O_coord[1]),  '{:07.3f}'.format(O_coord[2])]

    print("HOH_{} --> Angle = {}, O-H1 = {}, O-H2 = {}, O = {}, H1 = {}, H2 = {}".format('{:03d}'.format(i+1),  '{:.1f}'.format(HOHangle),
...
with open(args.input_pdb) as file:
    for line in file:
        if line.startswith("HETATM") and line[17:20].strip() == "HOH" and line[12:16].strip() == "O":
            ...
            Resi    = line[17:20]
            ChainID = line[21]
            ResID   = line[22:26].strip().zfill(4) 

            ChainRes_ID = ChainID + ResID + "_"
            Water = Resi + " " + ChainID + ResID
            print("\n") 
            print(Water, "---> Oxygen Coor  = ", oxygen_coordinates)
