...
import os.path
import numpy as np
###data_adv.py add extra.tpl reading in addition to extra.ftpl, no need to change extra.tpl to extra.ftpl format ---Cai
...
class Env:
    ...
    def read_extra(self):
      """Read extra.tpl."""
      if self.runprm["EXTRA"][-4:]=="ftpl":
        #print "ftpl",self.runprm["EXTRA"]
        self.load_tpl(self.runprm["EXTRA"])

      else:
        #print self.runprm["EXTRA"]
        lines=open(self.runprm["EXTRA"]).readlines()
        for line in lines:
             fields = line.split()
             if len(fields) != 3:
                continue
