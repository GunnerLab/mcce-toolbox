...
if __name__ == "__main__":
    ...
    for f in range(0, 100, 2):
        inputf = '/archive/yzhang/PS2/quinone/md_traj/traj_%s/frame%s_%s.pdb' % (traj_index, f, traj_index)
        resn = ['HOH', 'DLP', 'BGA', 'SOD', 'POP', 'SQD', 'BCR', 'CLA']
        prot = Protein()
        ...
        n_steps = [12, 48, 100]
        for n in n_steps:
            #print("   Layered strip off surface %s at precision +-%.3f ..." % (" ".join(resn), 1.0/n))
        ...
        filename, extension = os.path.splitext(inputf)
        if extension == ".pdb":
            fname = "%s-stripped.pdb" % filename
        else:
            fname = "%s-stripped.pdb" % inputf
        prot.writepdb(fname)
        filename, extension = os.path.splitext(inputf)
        #prot.writesas("%s.acc" % filename)
	#print('frame%s_%s.pdb finished' % (f, traj_index))
